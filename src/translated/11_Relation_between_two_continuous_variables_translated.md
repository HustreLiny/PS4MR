...
计算秩相关系数有两种常用方法，一种是斯皮尔曼（Spearman）方法，另一种是肯德尔（Kendall）方法。一般来说，计算斯皮尔曼的$r_{s}$（常称为斯皮尔曼的$\rho$）比计算肯德尔的$\tau$更简单，因此这里使用斯皮尔曼系数。计算过程见第11.7节。实际上，斯皮尔曼秩相关系数$r_{s}$与对观察值秩计算的皮尔逊相关系数$r$完全相同。  
...
<table><tr><td>受试者</td><td>年龄</td><td>秩</td><td>脂肪百分比</td><td>秩</td></tr><tr><td>1</td><td>23</td><td>1.5</td><td>9.5</td><td>2</td></tr><tr><td>2</td><td>23</td><td>1.5</td><td>27.9</td><td>7</td></tr><tr><td>3</td><td>27</td><td>3.5</td><td>7.8</td><td>1</td></tr><tr><td>4</td><td>27</td><td>3.5</td><td>17.8</td><td>3</td></tr><tr><td>5</td><td>39</td><td>5</td><td>31.4</td><td>11</td></tr><tr><td>6</td><td>41</td><td>6</td><td>25.9</td><td>5</td></tr><tr><td>7</td><td>45</td><td>7</td><td>27.4</td><td>6</td></tr><tr><td>8</td><td>49</td><td>8</td><td>25.2</td><td>4</td></tr><tr><td>9</td><td>50</td><td>9</td><td>31.1</td><td>10</td></tr><tr><td>10</td><td>53</td><td>10.5</td><td>34.7</td><td>16</td></tr><tr><td>11</td><td>53</td><td>10.5</td><td>42.0</td><td>18</td></tr><tr><td>12</td><td>54</td><td>12</td><td>29.1</td><td>8</td></tr><tr><td>13</td><td>56</td><td>13</td><td>32.5</td><td>12</td></tr><tr><td>14</td><td>57</td><td>14</td><td>30.3</td><td>9</td></tr><tr><td>15</td><td>58</td><td>15.5</td><td>33.0</td><td>13</td></tr><tr><td>16</td><td>58</td><td>15.5</td><td>33.8</td><td>14</td></tr><tr><td>17</td><td>60</td><td>17</td><td>41.1</td><td>17</td></tr><tr><td>18</td><td>61</td><td>18</td><td>34.5</td><td>15</td></tr></table>  
...
当然，这种接近并非总是如此。当散点图中的数据偏离椭圆形状时，两种方法往往会有所不同。由于这表明不适合计算皮尔逊相关系数$r$，因此当$r$和$r_{s}$明显不同的时候，应使用$r_{s}$。实际上，我们不会同时计算$r$和$r_{s}$，而是根据散点图的形态选择方法。秩相关可以用于任何类型的分布模式，且优点在于它不仅评估线性关联，更能反映更广泛的关联。例如，$r_{s}$的值在对任一变量进行对数变换后保持不变。然而，我之前对相关分析的所有警告同样适用于秩相关。  
...
纬度的相关系数为0.61。牛奶消费与纬度的秩相关为0.92，牛奶消费与DZ双胞胎率的秩相关为0.61。我们可以计算在控制牛奶消费（M）后的纬度（L）与DZ双胞胎率（T）之间的偏相关，结果为$r_{s}(LT|M) = 0.18$（参见11.7.3节）。这个较小的值提示，DZ双胞胎率与纬度之间观察到的关联，可能部分由牛奶消费解释。对这类国际相关性的解释是  
...
<table><tr><td></td><td>粘度</td><td>红细胞压积(PCV)</td><td>纤维蛋白原</td></tr><tr><td>红细胞压积(PCV)</td><td>0.8788</td><td></td><td></td></tr><tr><td>纤维蛋白原</td><td>0.4573</td><td>0.4155</td><td></td></tr><tr><td>蛋白质</td><td>-0.1011</td><td>-0.1575</td><td>-0.0512</td></tr></table>  
...
我们不能使用通常的表格来评估该相关系数，因为这里的原假设是相关系数为1，而非0。更容易考虑相关系数$r_{N}$的平方，记为$r_{N}^{2}$，称为$W'$。表B12展示了如何评估观察到的$W'$值。  
...
表11.5显示了一些血糖数据，将在本章后面使用，数据按升序排列。表中还显示了使用第7.5.2节公式计算的期望累计频率$(P_{i})$及对应的正态分数。原始数据与正态分数的相关系数为0.9772，因此$W'$的值为$0.9772^{2}=0.955$。根据表B12，我们得到$P>0.2$，说明数据与来自正态总体的样本相符。  
...
<table><tr><td>患者（i）</td><td>血糖值</td><td>Pi</td><td>正态分数</td></tr><tr><td>1</td><td>4.2</td><td>0.026</td><td>-1.947</td></tr><tr><td>2</td><td>4.9</td><td>0.067</td><td>-1.498</td></tr><tr><td>3</td><td>5.2</td><td>0.108</td><td>-1.236</td></tr><tr><td>4</td><td>5.3</td><td>0.149</td><td>-1.039</td></tr><tr><td>5</td><td>6.7</td><td>0.191</td><td>-0.875</td></tr><tr><td>6</td><td>6.7</td><td>0.232</td><td>-0.732</td></tr><tr><td>7</td><td>7.2</td><td>0.273</td><td>-0.603</td></tr><tr><td>8</td><td>7.5</td><td>0.314</td><td>-0.483</td></tr><tr><td>9</td><td>8.1</td><td>0.356</td><td>-0.370</td></tr><tr><td>10</td><td>8.6</td><td>0.397</td><td>-0.261</td></tr><tr><td>11</td><td>8.8</td><td>0.438</td><td>-0.156</td></tr><tr><td>12</td><td>9.3</td><td>0.479</td><td>-0.052</td></tr><tr><td>13</td><td>9.5</td><td>0.521</td><td>0.052</td></tr><tr><td>14</td><td>10.3</td><td>0.562</td><td>0.156</td></tr><tr><td>15</td><td>10.8</td><td>0.603</td><td>0.261</td></tr><tr><td>16</td><td>11.1</td><td>0.644</td><td>0.370</td></tr><tr><td>17</td><td>12.2</td><td>0.686</td><td>0.483</td></tr><tr><td>18</td><td>12.5</td><td>0.727</td><td>0.603</td></tr><tr><td>19</td><td>13.3</td><td>0.768</td><td>0.732</td></tr><tr><td>20</td><td>15.1</td><td>0.809</td><td>0.875</td></tr><tr><td>21</td><td>15.3</td><td>0.851</td><td>1.039</td></tr><tr><td>22</td><td>16.1</td><td>0.892</td><td>1.236</td></tr><tr><td>23</td><td>19.0</td><td>0.933</td><td>1.498</td></tr><tr><td>24</td><td>19.5</td><td>0.974</td><td>1.947</td></tr></table>  
...
一种看待相关性的方法，有助于抑制过度热情，是计算 $100r^{2}$，即由两个变量之间的关联“解释”的数据变异性的百分比。因此，相关系数为0.7意味着大约一半（49%）的变异性可以归因于观察到的关联，依此类推。这个概念与第11.13.6节和第12章中讨论的回归方差分析相吻合。计算相关系数的置信区间也可能有用，特别是对于小样本来说，置信区间会较宽。  
...
相关性常被用作探索性方法，以调查多个变量之间的相互关系，为此最明显的做法是使用假设检验。虽然原则上没问题，但这种方法常被过度使用。问题在于，即使变量数量适中，相关系数的数量也很大—10个变量产生45个$r$值，20个变量产生190个。仅凭偶然，约有1/20的相关系数会在5%显著性水平下显著，这些无法与真实关联区分开。此外，在5%显著性水平下显著的相关系数大小依赖于样本量。在大样本中，即使有几个相关系数约为0.2到0.3的显著值，这些相关性也不太可能非常有用。当确实没有先验假设时，这种观察大量变量的方法可能有帮助，但显著关联确实需要在另一组数据中得到确认，才能被信赖。  
...
变量。我们可以将这些视为响应变量（或结果变量）（Vcf）和预测变量（血糖）。这两者通常分别称为因变量和自变量，这两个术语容易混淆，但其含义是指一个变量依赖于另一个变量。响应变量总是绘制在垂直轴，即 $Y$ 轴上，预测变量绘制在水平轴，即 $X$ 轴上，如图11.9所示。  
...
其中 $b$ 是斜率，$a$ 称为截距，因为它是回归线与 $Y$ 轴的交点处的拟合值，即 $X=0$ 时的 $Y$ 值。在大多数医学应用中，$a$ 的实际意义不大，因为 $X$ 变量通常不可能接近零，例如血压或身体尺寸测量值。  
...
【1】因变量 $Y$（本例中的 Vcf）对于预测变量 $X$ 的每一个取值应服从正态分布；2. $Y$ 的变异性（用方差或标准差衡量）应在每个 $X$ 值处保持一致；3. 两变量之间的关系应为线性。  
...
与相关分析不同，回归分析不要求两个变量均为随机变量：如果预测变量 $(X)$ 的取值是由实验者选择的（如有时的情况），回归分析仍然有效。且 $X$ 的取值不必近似正态分布。  
...
如果上述三个假设成立，则残差应服从均值为零的正态分布。如果将残差绘制对 $X$ 值的散点图，点应均匀分布于所有 $X$ 值处。我建议常规制作此图。图11.11展示了残差图的三种可能情况：(a) 假设成立；(b) 随着 $X$ 增大，残差变异性增加；(c) 残差与 $X$ 值呈曲线关系。图 (b) 表明数据 $(Y)$ 可能需要对数转换，图 (c) 表明 $X$ 和 $Y$ 之间存在非线性关系（见11.12.2节）。有时不同问题会同时出现，此时对 $Y$ 变量进行对数或其他转换可一次性解决所有问题。  
...
检验给出 $W^{\prime} = 0.907$ $(\mathbf{P} = 0.03)$，表明存在一定程度的非正态性。图11.13表明这不是一个严重的问题，但如果担心的话，可以尝试对Vcf进行对数转换。使用 $\log_{e}$ Vcf进行回归分析后的残差的 $W^{\prime}$ 值为 $0.94$ $(\mathbf{P} = 0.12)$。  
...
斜率是回归分析中最重要的参数，因为它反映了两个变量之间关系的强度。拟合线的斜率为0.022，意味着我们估计每增加1单位（即1 mmol/l）空腹血糖，Vcf平均增加0.022%。斜率的标准误为0.0105。估计斜率 $^b$ 的处理方式类似于样本均值。我们可以计算斜率的置信区间，并检验斜率为零的假设，即Vcf与血糖无关系。第11.13节描述的计算得出斜率的95%置信区间为0.000至0.044；检验统计量为 $t = 2.10$，$\mathbf{P} = 0.05$。按照常规标准，斜率刚好显著不同于零。通常置信区间更具信息量，显示数据既兼容Vcf与血糖无关系，也兼容关系强度是观察值两倍的情况。  
...
拟合回归线解释了因变量$(Y)$变异的一部分，残差表示未解释的变异量。回归分析可用方差分析表展示，类似第9章所示。表11.7显示了血糖对Vcf回归的方差分析表。该表的推导见11.13.6节。许多软件包以此方式呈现结果。此格式易于扩展到复杂模型，详见第12章。需注意两点：首先，作为同一分析的另一种展示方式，$P$值与斜率的$P$值相同。实际上，$F$统计量是之前$t$统计量的平方$(4.41=2.10^{2})$。其次，残差均方(0.0470)是残差的方差，即残差标准差的平方。  
...
残差标准差表示回归线未解释的变异，是拟合优度的测量，单位与测量单位相同。更通用的拟合优度评估方法是考虑模型解释的总变异比例，通常通过回归解释的平方和占总平方和的百分比来衡量。根据表11.7，该值为$0.2073 / 1.1934 = 0.17$，即17%。该统计量称为$R^{2}$。  
...
<table><tr><td>变异来源</td><td>自由度</td><td>平方和</td><td>均方</td><td>F值</td><td>P值</td></tr><tr><td>回归</td><td>1</td><td>0.2073</td><td>0.2073</td><td>4.41</td><td>0.048</td></tr><tr><td>残差</td><td>21</td><td>0.9861</td><td>0.0470</td><td></td><td></td></tr><tr><td>总计</td><td>22</td><td>1.1934</td><td></td><td></td><td></td></tr></table>  
...
Vcf 与血糖之间相关系数的平方。该概念可扩展至更复杂的模型，下一章将再次讨论。这里较低的 $R^{2}$ 值表明，尽管斜率在统计学上显著，但 Vcf 的大部分变异性并未被血糖水平的变化所解释。  
...
如果我们有来自两个组的受试者数据，可以分别拟合回归线，然后比较两条线的斜率，看它们是否大致相似。可以获得差异的置信区间或进行显著性检验。如果两条线可视为具有相同斜率，则可以拟合两组数据具有相同斜率（即平行）的回归线。两条线之间的垂直距离即为调整了 $X$ 变量分布差异后，两组中 $Y$ 变量均值的差异。这种分析称为协方差分析。更多细节见 Altman 和 Gardner (1989)。协方差分析（第12.4.1节也有讨论）可扩展到超过两组的观察数据。  
...
多项式回归是多元回归的特例，用于描述（或“建模”）一个结果变量与单一预测变量之间的非线性关系。变量 $X$ 和 $Y$ 之间的线性关系对应回归方程 $Y = a + bX$。该思路可通过模型 $Y = a + bX + cX^{2}$ 扩展到非线性关系。该模型称为二次曲线，认为结果变量 $(Y)$ 不仅依赖于预测变量 $(X)$，还依赖于其平方 $(X^{2})$。通过这种方式，我们得到 $Y$ 与 $X$ 之间的曲线关系，尽管（如上所述）该模型是多元回归的特例，预测变量为 $X$ 和 $X^{2}$。  
...
出生体重 $(\mathbf{kg}) = -22.693 + 1.2122 \times \mathbf{age} - 0.014102 \times \mathbf{age}^2$，其中“age”是孕周数。该曲线见图11.16。  
...
回归分析将以表11.6中糖尿病患者的数据为例。我们希望回归线能根据血糖预测Vcf（环向缩短速度），因此$X$（预测变量）是血糖，$Y$（结果变量）是Vcf。  
...
参数$a$和$b$的估计值易于获得。设观测数据为 $x_i$ 和 $y_i$（$i=1, \ldots, n$），可证明回归线必须经过数据均值点$(\bar{x}, \bar{y})$。估计的斜率由下式给出：  
...
但该公式不应在计算机程序中使用，因为舍入误差可能导致不准确。用于此目的应仅使用上面给出的第一个 $b$ 的公式。  
...
因此，对于任意 $X$ 值，比如 $x_0$，由方程预测的 $Y$ 拟合值为  
...
其中  $y_{0} - y_{\hat{t}}$  即为该个体的残差。最小二乘法拟合的直线是通过最小化残差平方和  $\Sigma (y_{i} - y_{\hat{t}})^{2}$  实现的，但我们更关心的是残差的方差，其计算公式为  
...
该表达式的平方根，即残差标准差  $s_{\mathrm{res}}$ ，将在后续计算中使用。  
...
$$  
\begin{array}{c}{{s_{res}=\sqrt{0.04696}}}\\ {{=0.2167.}}\end{array}  
...
其中 $t$ 服从自由度为 $n - 2$ 的 t 分布（即与残差相关的自由度）。  
...
#### (b) 给定 $X$ 时的估计 $Y$  (b) Estimated Y for a given  $X$  
...
我们可以获得任意血糖水平下预测的 Vcf 均值的 95% 置信区间。如果  $y_{fit}$  是回归方程预测的 Vcf 均值，则  $y_{fit}$  的标准误为  
...
$$  
s e(y_{fit}) = 0.2167\sqrt{\frac{1}{23} + \frac{(x_{0} - 10.37)^{2}}{429.704}}  
s e(y_{f i t}) = 0.2167\sqrt{\frac{1}{23} + \frac{(x_{0} - 10.37)^{2}}{429.704}}  
$$  

其中  $x_{0}$  是血糖值。因此，对于血糖为  $14.5\mathrm{mmol / l}$ ，回归方程估计的均值 Vcf 为  
...
$$  
s e(y_{fit}) = 0.2167\sqrt{\frac{1}{23} + \frac{(14.5 - 10.37)^{2}}{429.704}}  
s e(y_{f i t}) = 0.2167\sqrt{\frac{1}{23} + \frac{(14.5 - 10.37)^{2}}{429.704}}  
$$  

我们利用上述公式计算估计值 1.419 的置信区间。根据表 B4，21 自由度下  $t_{0.975}$  值为 2.080，因此 95% 置信区间为  
...
截距通常不太引人关注，但可以使用上一节中的公式为截距 $a$ 计算置信区间，即当 $X=0$ 时 $y_{fit}$ 的置信区间。  
...
由于个体数据围绕拟合线的散布更为直接相关，$95\%$ 预测区间比拟合线的 $95\%$ 置信区间要宽得多。  
...
对于血糖值为 $14.5 \mathrm{mmol/l}$ 的个体，其 Vcf 值的估计标准差为  
The estimated standard deviation of Vcf values for individuals with a blood glucose of  $14.5 \mathrm{mmol / l}$  is  

$$  
\begin{array}{l}{{s_{pred}=0.2167\sqrt{1+\frac{1}{23}+\frac{(14.5-10.37)^{2}}{429.704}}}}\\ {{=0.225\%/\mathrm{sec}.}}\end{array}  
...
或 $0.95$ 到 $1.89\%/\mathrm{sec}$，这明显比均值的置信区间宽得多，可以通过比较图11.14和图11.15看出。  
...
### 11.13.5 斜率 $b$ 的假设检验  11.13.5 Hypothesis test for  $b$  

我们已经看到估计斜率 $b$ 的标准误为 $se(b) = s_{res} / \sqrt{S_{xx}}$，因此我们可以通过计算 $b / se(b)$ 来检验假设 $b = 0$。该比值与自由度为 $n - 2$ 的 $t$ 分布进行比较。  
...
血糖。我们只需将估计的 $b$ 除以其标准误，然后将结果与相应的 $t$ 分布值进行比较。因此我们有  
...
$$  
t = b / se(b) = 0.02196 / 0.0105 = 2.10。  
t = b / s e(b) = 0.02196 / 0.0105 = 2.10.  
$$  

该值与自由度为21的$t$分布进行比较，$t_{0.975}$的值为2.08。因此，斜率在5%的显著性水平上刚好显著地不同于零。  
...
因变量 $Y$ 的总平方和为 $S_{yy}$（自由度为 $n - 1$），回归平方和为 $(S_{xy})^{2} / S_{xx}$（自由度为1）。残差平方和（自由度为 $n - 2$）可通过相减获得。  
...
对于血糖数据，总平方和为1.1934，回归平方和为 $9.439^{2} / 429.704 = 0.2073$。这些结果见表11.7。  
...
如第8章所述，一组观测值的变异性部分可归因于已知因素，部分则来自未知来源；后者通常称为“随机变异”。在线性回归中，我们观察响应变量的变异性有多少可归因于预测变量的不同取值，拟合线两侧的散点显示了未解释的变异性。由于这种变异性，拟合线仅是总体中变量关系的估计值。与其他估计（如样本均值）类似，估计的斜率和截距 $b$ 和 $a$ 也存在不确定性。斜率 $b$ 的置信区间反映了估计关系强度的不确定性，而整条线的置信区间和个体预测区间则展示了变异性的其他方面。后者尤其有用，因为回归常用于对个体进行预测。  
...
这种外推是不合理的，因为我们没有关于观察数据范围之外关系的证据。统计模型只是一个近似。例如，很少有人相信真实关系恰好是线性的，但线性回归方程被视为对观察数据的合理近似。在观察数据范围之外，不能安全地使用相同的方程。因此，我们不应使用图11.14中显示的回归线来预测血糖值在4到 $20 \mathrm{mmol / l}$ 范围之外的Vcf。  
...
当预测变量的值由实验者选择时，回归分析是有效的，这在实验室实验中很常见。此外，如前所述，$X$ 变量不必服从正态分布。然而，如果存在一个远离数据主体的 $X$ 值，且对应的 $Y$ 值也极端，该观测点可能对回归线位置产生过大影响。图7.2中给出了一个例子。  
...
单因素方差分析中，针对三个或更多组的趋势检验在第9.8.5节和9.9.2节中已有描述。给各组赋予分数 $l_{i}$，并将组间平方和分解为线性和非线性部分。线性趋势检验几乎等同于对结果变量进行分数的回归分析，但不完全相同。  
...
其区别在于方差分析还用一个自由度检验组间的非线性变化，但本质上该方法是线性回归分析。回归线的斜率等于 $L / \Sigma l_{i}^{2}$，对应于响应变量随分数单位变化的变化量。该统计量比依赖分数值的统计量 $L$ 更有用。  
...
当只有两组，分数为 $-1$ 和 1 时，基于组分数的回归分析完全等同于两样本 $t$ 检验。回归线的斜率是两组均值差的一半。  
...
### 11.15.2  $2 \times k$ 频数表中的趋势检验  11.15.2 Trend in a  $2 \times k$  frequency table  

趋势卡方检验用于评估 $2 \times k$ 频数表中比例的趋势（见第10.8.2节）。该方法等同于对行变量（编码为0和1）与列分数进行回归。以表10.19中的剖宫产数据为例，若给六个鞋码组赋予1至6的分数，对351个观测值中行变量（编码0和1）与分数回归，斜率为 $-0.0302$（标准误0.0106），对应 $t = -2.86$（$\mathbf{P} = 0.0045$）。$\mathbf{P}$ 值与第10.8.2节中趋势卡方检验结果相同。然而，回归方法更具信息量，因为它给出从一组到下一组比例变化的估计值。这里估计为 $-0.03$（即鞋码每增 $\frac{1}{2}$，比例减少3%）。我们可用标准误按常规方法计算置信区间。  
...
应给出回归线方程及残差标准差。尽可能在图中同时展示回归线和原始数据散点图。回归线不应超出预测变量 $(X)$ 的观测范围。仅绘制回归线图并不比给出方程提供更多信息。  
...
（Campogrande 等，1977）。通常估计值 $a$ 会大于 $b$，但 $a$ 和 $b$ 往往报告到相同的小数位数。然而，进行预测时需要更精确的是斜率 $b$，而不是 $a$，因此 $b$ 应该至少与 $a$ 一样精确，甚至更精确。这里的精确度指的是“有效数字”的位数（即忽略开头的零）。因此，在前面给出的方程 $\mathrm{Vcf} = 1.10 + 0.0220$ 血糖中，截距和斜率均给出了三个有效数字。与上述的二次方程形成对比。  
...
在计算机程序的输出中，量 $s_{res} / \sqrt{n}$ 有时被称为“估计的标准误”（SEE）。这个名称不准确，因为它错误地暗示这是从回归线估计的任意值 $y_{fi}$ 的标准误。实际上，$s_{res} / \sqrt{n}$ 只是 $y_{fi}$ 在 $X$ 的均值处，即 $X = \overline{x}$ 时的标准误（见第11.13.3节）。如我们所见，随着远离均值，不确定性会增加。这种错误有时出现在发表的论文中，置信限被画成与回归线平行。更糟的是，有些程序将残差标准差 $(s_{res})$ 称为“估计的标准误”，这极具误导性。  
...
本章分别介绍了回归和相关，以阐明它们目的的不同。然而，从数学上看，这两种方法非常密切相关，如第11.7节和11.13节的公式所示。实际上，零相关的零假设的 $t$ 检验与回归分析中零斜率假设的检验完全等价—$ \mathbf{P} $ 值相同。许多计算机程序在执行回归分析时会自动提供相关系数，但应记住回归和相关是不同的方法，服务于不同的目的。除非确实对两者都感兴趣，否则通常不宜同时进行两种分析，这种情况并不常见。例如，如果我们知道阿尔巴尼亚人的肝硬化死亡率，并不会想用它来预测阿尔巴尼亚的猪肉消费。相反，一旦进行了更具信息量的回归分析，我们对 Vcf 与血糖水平之间的相关性就不再感兴趣。  
...
<table><tr><td>患者</td><td>乳酸</td><td>碳酸氢盐动脉水平变化</td><td>pH值</td><td>生存时间</td></tr><tr><td>19</td><td>16.2</td><td>-12.8</td><td>-0.15</td><td>72</td></tr><tr><td>20</td><td>17.5</td><td>-4.4</td><td>-0.09</td><td>96</td></tr><tr><td>21</td><td>2.7</td><td>-7.1</td><td>-0.21</td><td>192</td></tr><tr><td>22</td><td>4.4</td><td>-4.7</td><td>-0.05</td><td>336</td></tr><tr><td>23</td><td>4.8</td><td>-9.8</td><td>-0.05</td><td>456</td></tr><tr><td>24</td><td>9.0</td><td>-7.5</td><td>0.09</td><td>672</td></tr><tr><td>25</td><td>14.7</td><td>-7.2</td><td>-0.23</td><td>768</td></tr><tr><td>26</td><td>6.2</td><td>-4.2</td><td>-0.13</td><td>1080</td></tr><tr><td>27</td><td>18.4</td><td>-12.3</td><td>-0.12</td><td>2160</td></tr><tr><td>28</td><td>16.9</td><td>-8.6</td><td>-0.17</td><td>2160</td></tr><tr><td>29</td><td>26.0</td><td>-21.3</td><td>-0.32</td><td>24456*</td></tr></table>  
...
11.3 在回归分析实例（第11.10.1节）中，残差非正态性的  $W'$  检验得到了  $\mathbf{P} = 0.03$。  
...
(a) 利用表11.6中的数据，对  $\log_{e} Vcf$  与血糖进行回归分析。  
...
(c) 比较两个模型对一名空腹血糖为  $16 \mathrm{mmol / l}$  糖尿病患者预测的 Vcf 及其  $95\%$  预测区间。  
